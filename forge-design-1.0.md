...
**Author:** Rory  •  **Status:** v1.0 release candidate  •  **Target:** personal tool, OSS later

**This is the implementation contract.** Earlier versions (0.1 → 0.9) and their commentaries remain in the workspace as a record of how the design evolved. Implementers read only this document.

The 0.x series captured design evolution. 1.0 marks the end of pre-implementation design iteration. Future revisions (1.1, 1.2, ...) may be needed if Slice 0 (§16) discovers capability gaps that force scope narrowing; those continue to be fully standalone per §23.
...
1. Connector calls the underlying CLI with a system-prompt instruction to produce JSON matching `<schema>`.
...
5. **Second validation fails** → connector returns an adapter error. Caller decides (§7.6).
...
`config.schemaFallback.logValidatorErrors: true` (default) controls whether validator errors land in audit files.
...
- **SchemaFallback 2-attempt cap** — inside the connector when `schemaMechanism == SchemaFallback`. Retries only on schema-validation failures. Hard invariant.

**Layering:** `RetryOnProcessFailure(processRetries) { connector.reviewX(...) /* may use SchemaFallback internally */ }`.

A schema-validation failure that exhausts SchemaFallback returns an adapter error. The caller does **not** retry adapter errors. For refinery: advisory-failure path (§14.2). For design/code review: `NeedsHumanIntervention("reviewer schema fallback exhausted", <hint>)`.
...
9. Reviewer reviews `design.md` via `reviewer.reviewDesign(...)`. Uses `Native` or `SchemaFallback`. Wrapped by `config.<reviewer>.reviewProcessRetries`.
...
2. Validate `feature.designSessionId` per §11.0 step 5. `session <- driver.resumeStreamingSpec(feature.designSessionId.get)` → update `feature.designSessionId`. Forge sends a message referencing the feedback file. Driver questions handled per §7.8; answers to `.forge/specs/<feature>/audit/design-pr-feedback-r<n>-answers.md`. Bounded by `settle.designRevisionTimeoutSec`.
...
`reviewer.refine(...)` is wrapped by `config.<reviewer>.refineProcessRetries`. If those retries exhaust, OR if the reviewer returns an adapter error from exhausted SchemaFallback (§7.6):
...
Validate before any Scala is written. The matrix is 2 CLIs × 2 roles.
...
6. **Question mechanism:** at least one of:
   ...
   - **`HaltWithQuestion`** — ≥19/20 reliable structured halt + re-spawn.

**Reviewer role (both CLIs):**

7. **Schema-constrained output:** at least one of:
   - **`Native`** — CLI accepts and enforces an output schema. ≥19/20 valid outputs across three schemas.
   - **`SchemaFallback`** — ≥19/20 (95%) representative inputs per schema, per method.
...
Output: `slice-0-report.md` with pinned CLI versions, per-cell findings, fallback success-rate measurements, and the scope decision.
...
Per §16. Output: `slice-0-report.md` and any required scope changes (becoming v1.1, v1.2, etc.).
...
- `ClaudeConnector` and `CodexConnector`. Each declares `questionMechanism` and `schemaMechanism`.
- `SchemaFallback` protocol (§7.5) implemented once in `forge-agents`.
- `HaltWithQuestion` parsing + re-spawn loop (§7.3) in the orchestrator scaffolding.
- Integration tests:
  ...
  - HaltWithQuestion exercised with a contrived ambiguous prompt.
  - `resumeStreamingSpec` exercised end-to-end.
...
- Property tests:
  ...
  - **`feature.designSessionId` lifecycle per §6.1:** populated on `InteractiveSpec` spawn / design-revision resume, retained through every design-phase state, cleared on `DesignReady`.
...
Notes:
- SchemaFallback 2-attempt cap is a **protocol invariant** (not exposed in config).
- `reviewProcessRetries` and `refineProcessRetries` cover **process-level failures only** (§7.6).
...
- `<actor>.resume` — `{ sessionId, newSessionId }`. Also updates the projection.
...
| Process retries vs schema attempts? | Distinct layers. Adapter errors not retried at outer level. |
| Capability emulation? | Only `SchemaFallback`. Nothing else. |
| Mode pluggability? | Two modes (`ClaudeDriver`, `CodexDriver`). |
| Slice 0 fallback bar? | ≥19/20 per method, per schema. |
| Slice 0 capability gap? | Design-scope decision (wait / narrow scope / v1-blocking). |
...
| First thing to build? | Slice 0. Then Slice 1 (connectors). |
...
- Capability emulation beyond `SchemaFallback`.
- Fake config knobs.
...
- 0.1 → 0.9 documents and their commentaries remain in the workspace as a record of design evolution but are not required reading.
- Future revisions (1.1, 1.2, ...) prompted by Slice 0 findings or production lessons will also be standalone.
...
This design went through nine rounds of review and commentary (0.1 through 0.9 → 1.0). The reviewer was consistent across rounds. The pattern that worked:
...
The architecture stabilised at 0.5 and remained unchanged through 1.0. Five revisions without architectural movement was the signal that the work was done.
