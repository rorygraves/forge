...
**Author:** Rory  •  **Status:** draft v0.7  •  **Target:** personal tool, OSS later

**This is the first fully standalone spec.** Earlier versions (0.1 → 0.6) and their review commentaries remain in the workspace as a record of how the design evolved. Implementers should read only this document — nothing here references prior versions.
...
| Failure leaves feature in limbo | `NeedsHumanIntervention(reason, resumeHint)` is non-terminal; three typed resume paths |
...
forge/
  ...
  modules/
    ...
    forge-agents/        ← Connector trait, ClaudeConnector, CodexConnector, stream-json
...
- `baseSha` is recorded by BranchManager at piece-branch creation time and used by the base-freshness check (§9).
...
5. **Second validation fails** → connector returns an adapter error. Orchestrator transitions to `NeedsHumanIntervention("reviewer schema fallback exhausted", <phase-appropriate hint>)` with schema validation errors in the audit file.
...
### 7.7 Driver-question handling applies to *every* driver session

The mechanisms in §7.2–§7.3 apply to:
- **Spec consolidation** (§11.1 step 4) — driver writes design/manifest/piece files.
- **Design revision** (§11.2 step 11, §11.3 step 2) — driver revises after reviewer or PR feedback.
- **Implementation** (§11.4 step 4) — driver implements a piece.
- **Fix-up** (§11.6) — driver addresses CI/review failures.

For Claude-driver, all sessions use `Native`. For Codex-driver, all sessions use `HaltWithQuestion`. The re-spawn protocol carries the relevant prior context for each session type (the spec consolidation re-spawn carries the conversation so far; the implementation re-spawn carries design + piece spec + prior turn outputs; etc.).

### 7.8 Runaway detection — two bounds

Every driver session is bounded by **both**:

1. **Settle timeout** (`settle.<phase>TimeoutSec`) — wall-clock cap on the whole session. On expiry: SIGTERM, 5s grace, SIGKILL. Transition to `NeedsHumanIntervention("<phase> settle timeout", <hint>)`. This catches subprocesses that produce no output at all (stuck on stdin, hung in a tool call).
2. **Per-turn cost cap** (`maxTurnCostUsd`, default \$2) — checked after every `cost.update`. On breach: SIGTERM, 5s grace, SIGKILL. Transition to `NeedsHumanIntervention("turn budget exceeded", <hint>)`. This catches subprocesses that produce output but run away in cost (tool-use loops).

A driver that produces neither a question nor a completion is bounded by (1). A driver that produces lots of output cheaply is bounded by neither and may continue — that's correct: it's making progress.
...
- `createPieceBranch` returns `(branchName, baseSha)`; the FSM persists `baseSha` into the manifest piece entry.
...
`Deny` → `NeedsHumanIntervention("change collector denied <path>", RunAnotherFixup(p, prNumber))`. `Ask` → Q&A pane, default option `Deny`. `Allow` → staged. The stage plan is recorded in the action log and rendered into the PR body.
...
2. Spawn driver in streaming spec mode with `~/.forge/prompts/specify.<driver>.md`.
...
4. User types `/done` (intercepted by Forge, not sent to driver). Forge sends a final message asking the driver to write `design.md`, `manifest.json` (with piece IDs `p1, p2, ...` and `mode` field), one `pieces/<p>.md` per manifest entry, and to render `decomposition.md` from the manifest using `~/.forge/templates/decomposition.md.hbs` (with editable-region markers per §5.3).
5. Driver questions handled per §7.7 (Native or HaltWithQuestion depending on driver).
6. **Settle** = next `result` event after Forge's final message, with `settle.specTimeoutSec` (default 300s) hard timeout. On expiry: SIGTERM/SIGKILL, transition to `NeedsHumanIntervention("spec settle timeout", AbortOrAbandon)`.
...
9. Reviewer reviews `design.md` via its schema-constrained exec mode (`~/.forge/schemas/design-review.json`). Uses `Native` or `SchemaFallback` per the reviewer's declared `schemaMechanism`.
...
12. `verdict: request_changes` → resume driver design session, ask it to revise `design.md` + manifest + affected `pieces/*.md` addressing the blockers. Driver questions handled per §7.7. Loop to step 9. Max `config.maxDesignReviewRounds` (default 3) → `NeedsHumanIntervention("design did not converge", ReopenDesign(None))`.
...
2. Resume driver design session. Driver questions handled per §7.7.
...
1. `syncBase()`, then `createPieceBranch(feature, p.id)` returns `(branchName, baseSha)`. Persist `baseSha` into `manifest.pieces[i]`.
2. Spawn driver headless with `~/.forge/prompts/implement.<driver>.md`. Prompt: implement piece `<p.id>` to acceptance criteria; **do not commit — Forge will commit**.
...
4. Driver questions handled per §7.7. Always route to Forge Q&A pane regardless of mechanism.
5. Settle bounded by **both** `settle.implementTimeoutSec` (default 1800s) and `maxTurnCostUsd` (§7.8).
6. On settle:
   - ChangeCollector (§10.1) classifies changes.
   - Forge commits with `feat(<feature>): <piece title>`. Respects repo `commit.gpgsign`.
...
- Any required check failed: increment `piece.attempts` in the manifest. If `attempts <= config.maxFixupRounds`: write `<p>.failures.md`, FSM → `PieceCiFailed(attempt)` → fix-up (§11.6). Else → `NeedsHumanIntervention("piece <p> CI did not converge", RunAnotherFixup(p, prNumber))`.

From `PieceAwaitingReview`:
- Reviewer `approve` posted, no unresolved human `CHANGES_REQUESTED` → `PieceAwaitingMerge`.
- Reviewer `request_changes` posted → `PieceReviewFailed` → fix-up.
...
- New human comment or `CHANGES_REQUESTED` before merge → `PieceReviewFailed` → fix-up.
...
Fresh driver session, no resume. Prompt references `<p>.failures.md` (CI failure logs + unresolved review comments + previous failure reasons + PR URL/branch) and the unchanged piece spec. Driver questions handled per §7.7. Forge commits on settle via the same ChangeCollector path.

On `piece.attempts > config.maxFixupRounds` (default 3) → `NeedsHumanIntervention("piece <p> fix-up exhausted", RunAnotherFixup(p, prNumber))`. The PR is left open. `forge resume <feature>` after a human fix re-enters from `PieceAwaitingCi`.
...
3. **Per-turn breach (mid-turn):** if a single turn's accumulated cost exceeds `maxTurnCostUsd`, SIGTERM the subprocess, 5s grace, SIGKILL. Transition to `NeedsHumanIntervention("turn budget exceeded", <hint>)`. Catches runaway tool-use loops.

Together with the settle timeout (§7.8), every driver session is bounded by two independent guards.
...
Reviewer `exec` errors (network, schema-invalid after `config.<reviewer>.refineRetries` retries, sandbox failure) → log `harness.refinery_failed` → FSM → `PieceMerged`. Refinery is advisory; the piece is already merged with required CI green, that's the load-bearing signal. Don't strand the feature on advisory tooling.
...
- `Connector` trait per §7.1.
- `ClaudeConnector` and `CodexConnector` implementing it. Each declares its `questionMechanism` and `schemaMechanism`.
...
- BranchManager: create design branch, create piece branch (returning baseSha), push, create PR (parsing number from `gh pr create --json url`), preflight checks, snapshot tags.
...
| Required vs optional checks? | Branch-protection required + overlay; missing-overlay timeout transitions to `NeedsHumanIntervention`. |
...
| Work breakdown SoT? | `manifest.json` (machine); `decomposition.md` rendered with editable-region markers; `forge reconcile` imports edits. |
...
| Settle bound? | Every driver session bounded by both settle timeout and per-turn cost cap. |
...
- **Fake config knobs.** `schemaFallback.maxAttempts` removed; the cap is a protocol invariant.
...
- This spec is **standalone**. No section references prior version files. The 0.1 → 0.6 documents and their commentaries remain as a record of design evolution but are not required reading.
