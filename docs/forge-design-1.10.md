...
**Historical note (2026-06-13).** The 1.10 implementation deliberately shipped a same-CLI resolver because that matched the production wiring at the time. The product direction has since been clarified: same-CLI remains a valid pairing, but Forge must also support true cross-model validation. The live statement of that requirement is in `forge-design-1.16.md`, `forge-design-2.0.md`, and the roadmap; this historical revision is kept as the audit trail for how the role seam first landed.
