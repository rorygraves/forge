...
1. **Codex write-on-resume is unverified.** `execResumeArgv` deliberately passes **no** sandbox/approval flags (§7.10(c) — resume rejects session-scoped flags), and the spike's recall turn was read-only. Turn 1 wrote under `--sandbox workspace-write`; whether a *resumed* implement/fix-up turn can still write files is **untested**. D3-1 must verify a resumed Codex turn that edits files, and if it cannot, decide whether the driver's writes happen pre-settle only (so resume need not write) or whether codex config must carry the sandbox.
