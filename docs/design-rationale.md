...
### PH4-3. Containerised worker control channel needs TCP on macOS (Unix-socket bind mount fails) — deferred to 4.5
**Decision:** for the **containerised** topology, the worker↔daemon control channel must move from a bind-mounted Unix socket to **TCP** to work on the stated macOS host (contract §7). Deferred to **4.5**; **Slice-4.3 dogfood #8 runs on a Linux host** in the meantime. The host-process (4.2) topology is unaffected (no container boundary).
**Origin:** Task 4.3.6 dogfood prep (2026-06-07). Empirically: a container connecting to a *host-created* Unix socket over a bind mount on Docker Desktop for macOS fails with `OSError [Errno 95] Operation not supported` — the VM boundary does not pass host Unix sockets through. A socket on a *shared Docker volume* between two in-VM (Linux) containers **does** cross (verified). `ContainerRuntime` currently bind-mounts the daemon's host Unix socket into the worker container — correct on a Linux host, broken on macOS.
**Why the suites missed it:** `ContainerRuntimeSuite` asserts the *spec* (mounts/argv, no host home), `SupervisorContainerReconcileSuite`'s opt-in real-Docker test uses only `docker run`/`wait`/`kill` (no socket crossing), and `WorkerDaemonHandshakeSuite` exercises the worker-side seams over a host-local socket (no container). Only a real container *connecting back over the mounted socket* — the live dogfood — surfaces it; the host→container Unix-socket crossing is not portably automatable, which is itself why this is a dogfood-class finding.
**Ratified with the user:** 2026-06-07 (AskUserQuestion — "re-architect to TCP, defer to 4.5; run dogfood #8 on Linux"). Carry-forward in [`design-4.3.md`](design-4.3.md) §4; contract note in [`forge-design-2.0.md`](forge-design-2.0.md) §7 (transport caveat) + O9.
