...
> Why the live spec ([`forge-design-1.1.md`](forge-design-1.1.md)) looks the way it does. Each item names a decision, what was rejected, and what to read in v1.1 for the resulting contract.
>
> The design went through eleven iterations (0.1 → 0.9 → 1.0 → 1.1) with delta-only commentaries at each step. Those commentaries have been removed from the tree; this document preserves the non-obvious rationale a future contributor would want before re-litigating a settled question. Decisions already explicit in v1.1 §22 (rejects) or §20 (v2 candidates) are not repeated here.
