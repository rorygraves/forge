...
> Why the live spec ([`forge-design-1.6.md`](forge-design-1.6.md)) looks the way it does. Each item names a decision, what was rejected, and what to read in the live spec for the resulting contract. Older entries reference §-numbers in v1.1 / v1.2 / v1.3 / v1.4 / v1.5 — those sections survived intact into v1.6 (the numbering is preserved across revisions) unless explicitly noted.
>
...
**Action required for v1:** ~~v1.3 spec correction needs to either (i) drop the §7.10(a) "applies to resume" claim and shift the role-framing responsibility to the orchestrator's resume message, or (ii) widen the trait to carry the path and accept the call-site churn.~~ **CLOSED in Slice 1.4b Task 1.4.14 (2026-05-30) — option (ii).** [`forge-design-1.3.md`](forge-design-1.3.md) §7.1 / §7.10(a) widens the trait to `resumeStreamingSpec(sessionId: String, systemPromptPath: os.Path, message: String)`. `ClaudeConnector` ignores the path (`--resume` restores the prompt server-side; the resume argv still omits `--system-prompt-file`); `CodexConnector` re-prepends it via `CodexPrompt.withSystemBlock`, symmetric with `runStreamingSpec`. The call-site churn the slice-1 note feared turned out tiny: one production site (`RealSideEffects.resumeDesign`, which passes `promptPath("specify")` — the same driver prompt the spawn used) plus test fakes. Rejected-then-chosen: the slice-1 "rejected (a)" applies only in the abstract; with the orchestrator now real, keeping connector-specific framing **inside** the connector (rather than leaking "Codex needs in-message framing, Claude doesn't" up into `RealSideEffects` per option (i)) is what preserves the §7.1 connector seam. Coverage: `CodexConnectorSuite` (fake-CLI resume asserts the system block reaches the combined prompt), `ClaudeConnectorSuite` (resume ignores a non-existent prompt path), `RealSideEffectsSuite` (resume call site passes `specify.<cli>.md`); real-CLI bar stays the Task 1.4.7 `ReviewerRegressionSuite`.
**In the spec today:** ~~v1.2 §7.10(a) (states the broken contract), §7.1 (the trait signature that doesn't carry the path).~~ Resolved in v1.3 §7.10(a) / §7.1. Surfaced as a PR-C reviewer comment; closed in Task 1.4.14.
...
### C19. Headless driver `--resume` restores conversation context on both connectors — D3 respawn-avoidance is viable (design-3.5 D3-0 spike, runtime finding)
**Finding (D3-0 spike, `DriverResumeSpikeSuite`, real CLIs 2026-06-01):** The "D3 large half" (driver-respawn-avoidance, roadmap §3.5 / [`design-3.5.md`](design-3.5.md)) rests on one assumption that was untested: that a **headless** driver turn (`claude -p …` / `codex exec …`), interrupted and then resumed **from a fresh process**, continues with its prior context rather than re-exploring. Only the *spec* phase had a resume path (`resumeStreamingSpec`, streaming mode); the implement/fix-up drivers are one-shot headless runs with **no `--resume` method**, so `claude -p … --resume <sid>` / `codex exec resume … <thread-id>` for a *driver* turn was a wire combination Forge had never run. The spike exercised it directly (codeword planted in turn 1, recalled in a fresh-process turn 2). Result: **both connectors GO.**
- **Claude** (`claude -p … --resume <sid>`, **no** `--system-prompt-file` — restored server-side): the resumed turn replied the exact planted codeword (`ZARQ-9173-WIDGET`), the session id was preserved across the fresh process (§6.1), the recall turn cost only ~\$0.036, and a separate *killed-mid-turn* turn (kill at 8s) resumed and **continued the in-flight task to a clean `Result`** — i.e. it picked up exploration, not restarted it. This is the realistic D3 crash shape and it works.
- **Codex** (`codex exec resume --json <thread-id> <combined>`): turn 1 and turn 2 shared the identical `thread_id` (`019e82fd-…`) and the resumed turn replied the exact codeword — so `exec resume` restores the conversation **despite being CLI-stateless** (the thread carries the history; the system block is re-prepended per C14, but the codeword lived only in the prior *user* turn, so recall proves thread-context restoration, not the system block).

**Decision:** Proceed with D3-1 (build the headless-resume connector seam for **both** connectors), then D3-2 (worktree-safety classifier) and D3-3 (orchestrator resume-instead-of-respawn, default-on once safe). The spike's inline resume argv (built from the public `ClaudeConnector.{IsolationFlags,OutputFlags,driverPermissionFlags}` / `CodexConnector.execResumeArgv` + `StreamingDriver.fromSubprocess`) is the prototype for D3-1's method.

**Watch items surfaced by the spike (carry into D3-1/D3-3):**
1. ~~**Codex write-on-resume is unverified.**~~ **RESOLVED (D3-1, 2026-06-01).** `execResumeArgv` deliberately passes **no** sandbox/approval flags (§7.10(c) — resume rejects session-scoped flags), and the spike's recall turn was read-only, so whether a *resumed* implement/fix-up turn could still write was untested. The D3-1 `forge-it` `DriverResumeSeamSuite` ran a real-CLI resumed Codex turn that creates a file via `Connector.resumeHeadlessDriver` → the file landed (24.5s, against codex 0.133.0). So Codex resolves the sandbox from the **sticky thread settings** on resume (the original `exec` ran under `--sandbox workspace-write`); no sandbox-on-resume workaround is needed and the driver may write pre- or post-resume. The same suite confirmed Claude write-on-resume. The behaviour is pinned by the suite (opt-in `FORGE_IT_RUN_RESUME_SEAM=1`) so a future codex sandbox-semantics change surfaces as a test failure.
...
**Walked (Slice 1.4b Task 1.4.15 O5, 2026-05-30):** no perf cliff surfaced — Slice 1.4b ships no throughput-benchmark harness (the orchestrator e2e suites assert correctness, not steady-state write rate), and at the single-digit-actions-per-second peak the spec already cites, the per-append `SYNC` cost is dominated by orchestrator/network latency rather than fsync. **Disposition: keep `CREATE,APPEND,SYNC` as the v1.3 baseline.** The per-batch `force()`-after-non-sync fallback stays documented but un-adopted; the first lived-cadence observation point is the Task 1.4.16 MVP run, and any cliff there (or in Phase 2) re-triggers option (a)/(b).
...
**Action required for v1:** ~~`design-1.4.md` (Slice 4) must open with this carry-forward in its §4 list and the writer-side atomic-merge test must be a gating check on the orchestrator-loop sub-PR.~~ **CLOSED in Slice 1.4b Task 1.4.11 (2026-05-29).** `OrchestratorAtomicMergeSuite` (`modules/forge-app/src/test/scala/io/forge/app/orchestrator/`) drives a feature through the real `Orchestrator` loop to `PieceAwaitingMerge`, injects a crash on the `audit.piece_merged` action-log append, and pins the §11.5-step-1 writer order directly: (1) the manifest is already persisted `merged`, (2) the action-log batch (fsm.transition + audit) never landed, and — the invariant the e2e suite did not assert — (3) the state cache **lags behind** the manifest, still holding the pre-transition `PieceAwaitingMerge` state (because `cache.save` runs only after the crashed audit append). A direct `RebuildState.run` restart then recovers via reconcile case (c) to `Refining`.
**In the spec today:** v1.2 §11.5 step 1 (the writer contract), §17 slice-2 invariant 14 (the test obligation). Reader side: `modules/forge-core/src/test/scala/io/forge/core/property/F13AtomicMergeCrashSuite.scala`. Writer side: `modules/forge-app/src/test/scala/io/forge/app/orchestrator/OrchestratorAtomicMergeSuite.scala` (Slice 1.4b Task 1.4.11).
...
**CLOSED (Slice 1.4b Task 1.4.12, 2026-05-29):** option **(i)** landed. `Fsm.transition` now handles all 7 `SessionPhase` variants — `SettleTimeout(SessionPhase.DesignReview, _)` from `DesignReviewing` → `NHI(ReopenDesign(currentDesignPr))`, `SettleTimeout(SessionPhase.CodeReview, _)` from `PieceAwaitingReview` → `NHI(RunAnotherFixup(p, pr))`, `SettleTimeout(SessionPhase.Refine, _)` from `Refining` → `NHI(RunAnotherFixup(p, pr))` (the latter clears the stale `currentPieceSessionId` per §6.1). Each hint mirrors its phase's existing recovery rule (the DesignRevision / fix-up / `hintFromState` paths). The orchestrator-side `Timeout` → `SettleTimeout` mapping landed at Task 1.4.10 (`Orchestrator.designReviewEvent` / `prReviewEvent` / `refineEvent`). Coverage: three new `ResumeHintCoverageSuite` rows (§B3(a) labels) pin each handler's NHI hint; full `forge-core` suite green (371).
...
**Walked (Slice 1.4b Task 1.4.15 O5, 2026-05-30):** moot for v1 as wired. The landed `Orchestrator` rebuilds via `RebuildState.run` **only at startup** (`modules/forge-app/src/main/scala/io/forge/app/orchestrator/Orchestrator.scala:74,100`) and persists steady-state via `cache.save` per transition (`Orchestrator.scala:185`); it does **not** call `verifyAgainstLog` on the loop at all (the method is currently exercised only by `FileStateCacheSuite`). So the "potentially after every FSM transition" worry the original entry flagged never materialised — the unconditional temp+rename+fsync runs at most once per command invocation (startup recovery / `forge rebuild-state`), which is negligible. **Disposition: keep the unconditional `cache.save` (and `RebuildState.run`'s unconditional step-6 save) as the v1.3 baseline.** Options (a)/(b) stay documented but un-adopted; re-trigger only if a future caller wires `verifyAgainstLog` onto a hot path or the Task 1.4.16 MVP run shows startup recovery firing often enough to matter.
...
**Walked (Slice 1.4b Task 1.4.15 O5, 2026-05-30):** no lived `forge run` cadence yet to judge the cliff against — the first is the Task 1.4.16 MVP run. The 3-consecutive default has not felt wrong in any e2e suite (which don't exercise sustained real rate-limiting). **Disposition: keep `consecutiveRateLimitsBeforeFailing = 3` as the v1.3 baseline.** It is already a `PRWatcherConfig` knob, so tightening/loosening is a config change, not a code change; re-trigger if the Task 1.4.16 MVP run or Phase-2 cadence shows the threshold mis-shaped.
...
### S3-4b. A transient `gh` server / network blip is a soft `PollResult.TransientError`, not a hard `Failed` (dogfood finding #5, 2026-06-04)
**Decision:** the §9 poll now has a transient-error twin of the S3-4 rate-limit soft-cliff. `RealPRWatcher.pollOnce` maps a `GhError.Transient` (a non-zero `gh` exit with no rate-limit / 404 / auth framing — the HTTP 503 / 502 / 504 / network-blip bucket) to the new **`PollResult.TransientError(err)`**, a *non-failing* event exactly like `RateLimited`: the stream sleeps `PRWatcherConfig.transientBackoff` and keeps polling, the orchestrator absorbs it (`pollResultToEvent` / `ciPollToEvent` return `None`, keep polling), and a per-watch consecutive counter promotes it to `PollResult.Failed(err)` only after `consecutiveTransientFailuresBeforeFailing` (default 3, mirroring the rate-limit cliff). The two counters are independent — a result of one kind resets the other — so an interleaving of transient + rate-limit doesn't pool toward a premature `Failed`. The *fatal* `GhError` flavours (`NotFound` / `Unauthorized` / `ParseFailure`) stay hard `Failed` → `HarnessError` → NHI: polling cannot fix a wrong PR number, a missing token, or decoder drift.
**Why:** dogfood #2 and #3 both hit a transient GitHub 503 on the `PieceAwaitingReview` PR-state poll *after* CI was already green; the watcher surfaced it as `Failed`, which the orchestrator hard-routed to `NeedsHumanIntervention(RunAnotherFixup)` — a spurious, misleading NHI that then needed an audit-log-trimming manual recovery. S3-4 already proved the right shape for the symmetric rate-limit case (absorb internally, escalate only on a sustained run); a 5xx blip is the same "GitHub is momentarily unavailable" condition and wants the same treatment. The §8.2 `FailureClassifier`/`RateLimit→BackOff` arm does *not* cover this — that classifies CI *check-failure logs*, a different domain from a `gh` subprocess HTTP error on the poll itself, which is already typed as `GhError` at the client boundary.
**Rejected:** (a) **route the poll error through the §8.2 `FailureClassifier`** — that scans CI log *text* for markers; a `gh` poll error is a typed `GhError`, so re-stringifying it to re-classify is a layering inversion. (b) **classify 503 specially in `RealGhClient.classify`** — unnecessary: a 503 already falls through to `GhError.Transient` (no rate-limit/404/auth framing), which is exactly the retry-worthy bucket; the fix belongs at the consumer's retry policy, not the wire classifier. (c) **absorb transients forever with no cliff** — a genuinely-down GitHub would poll silently forever; the N-consecutive promotion to `Failed` preserves the eventual-escalation guarantee.
**Action required:** none beyond the landed change; `transientBackoff` reuses the §18 `github.rateLimitBackoffMs` knob (both are "pause and retry" intervals), and `consecutiveTransientFailuresBeforeFailing` defaults to 3 — re-shape via config if lived cadence shows it mis-tuned (same posture as S3-4).
**In the spec today:** extends S3-4 + RL1; v1.2 §9 (`PRWatcher`), §18 (`rateLimitBackoffMs` reused for `transientBackoff`); `PollResult.scala` (`TransientError` case), `PRWatcherConfig.scala` (`transientBackoff` / `consecutiveTransientFailuresBeforeFailing` knobs), `RealPRWatcher.scala` (the second per-watch counter), `Orchestrator.scala` (`pollResultToEvent` / `ciPollToEvent` absorb arms); `PRWatcherTransientSuite` + `OrchestratorTransientPollSuite` + `GhErrorClassifierSuite` (503→Transient) pin it. Tracked as the resolved half of design-3.0.md **T6**.
...
**CLOSED (Slice 1.4b Task 1.4.12, 2026-05-29):** closed together with **S2-8** (see its CLOSED note). The `SessionMonitor` driver-phase carve-out stands as designed — reviewer/refine wall-clock caps remain in the `ReviewerCall` wrapper and never route through `SessionMonitor`. The FSM-side gap that mirrored this carve-out is now closed: `Fsm.transition` routes the three reviewer/refine `SettleTimeout` phases to phase-appropriate `NHI` hints (orchestrator maps `ReviewerOutcome.Timeout` → `FsmEvent.SettleTimeout` at Task 1.4.10; FSM handlers + `ResumeHintCoverageSuite` rows at Task 1.4.12).
...
**Task 1.4.17 close-out decision (2026-05-31): rolls forward to Phase 2.** The MVP-gate run (Task 1.4.16) surfaced that the action log records FSM transitions but **no per-run cost/latency at all** — so there is not yet data to judge whether reviewer spend cliffs warrant the connector-boundary widening. The dependency is the wrong way round to close inside Phase 1: cost must be *instrumented* before it can be *attributed* to the reviewer path. Phase-2 Slice 2.0 ("instrument before optimise", roadmap §3.1) wires the `cost.update` writer first; S4-3's reviewer-cost/kill-diagnostics widening is then re-evaluated against real attributed data. Until then the documented v1 limitation stands: the §12 budget caps are driver-session-only invariants, and the four reviewer/refine calls per piece are bounded by the wall-clock cap (proven real by `ReviewerCallSubprocessTimeoutSuite`).
...
**Slice 2.0 close-out (2026-05-31): instrumentation half now delivered; widening still deferred.** Slice 2.0 wired the driver-session cost/latency instrumentation (`cost.update` writer, `session.complete`, `forge stats`) — the data S4-3 was waiting on. The *reviewer-side* cost/kill-diagnostics widening (returning cost from the one-shot `review*`/`refine` calls so reviewer spend joins `Feature.cost`) is **not** part of Slice 2.0 and still wants attributed data from a multi-feature run before the connector-boundary change is worth it; it rolls forward to the same later Phase-2 tuning slice as [[S4-5]] (roadmap §3.5). The driver-session-only-budget limitation stands meanwhile.

**Slice 2.2 close-out (2026-06-05): the cost-widening half LANDED; `killError` half still deferred.** Slice 2.2 (roadmap §3.5; contract [`forge-design-1.16.md`](forge-design-1.16.md) §7.1/§12) took the chosen closure path (a): the `Connector` reviewer/sensor methods now return `Reviewed[A](value, cost: Option[Cost])` — the cost is read off the **same envelope** the verdict is decoded from (Claude `total_cost_usd`/`usage`/`modelUsage` via the shared `ClaudeEventParser.costFromResult`; Codex `turn.completed.usage` via `extractTurnTokens` + `priceTable.usdFor`). It surfaces on `ReviewerOutcome.Settled(result, cost)`, and the orchestrator writes a `cost.update` (`actor = "reviewer"`) folding feature+piece (never turn) — so **reviewer spend now joins `Feature.cost`** and the §12 caps are no longer driver-session-only. Two exceptions are documented: **`classifyFailure`'s cost.update *write* is deferred** (the connector boundary carries its cost, but wiring the orchestrator write needs the live cost-totals ref threaded through the CI-watcher + local-build-gate side-effect chains for the rarest reviewer call — `Unknown`-classification only; bounded under-count); **`profileRepo` is exempt** (out-of-band `forge profile`, no feature to attribute to). The S4-3 **second half** — the `killError`-on-`ReviewerOutcome.Timeout` observable-kill diagnostic — **stays deferred** (the connector still exposes no subprocess handle to the wrapper). Proven by `ReviewerCostExtractionSuite` + `OrchestratorReviewerCostSuite`. Roadmap §3.5 `[~]` stays open pending the whole-section review.

### S4-1. `PollBaseline`s persist to a sibling `.forge/state/<feature>.poll-baselines.json`, not the manifest (slice-1.4b Task 1.4.10)
**Decision (slice-1.4b Task 1.4.10):** The orchestrator's `PRWatcher` poll loop carries a `PollBaseline` (comment/review cursors + last-seen check-run ids) per watched PR. `Manifest` / `Piece` (`forge-core`) carry no baseline fields, so the baseline map (`Map[PrNumber, PollBaseline]` across the design PR + every piece PR) persists to a new gitignored sibling state file, addressed by `ForgePaths.pollBaselineFile(featureId)` → `.forge/state/<feature>.poll-baselines.json`. The §S3-7 round-2 contract (baseline advances only on `PollResult.Snapshot` arrival) is unchanged; this entry only fixes *where* the baseline lives.
**Why this is a deviation:** v1.2 §9 describes the poll baseline as orchestrator state but does not pin a persistence location, and §6 leaves `PrSnapshot` / `Manifest` without baseline-cursor fields.
**Why this is OK:** The manifest is the §4 committed source of truth; `PollBaseline` is local-only state that mutates on every gh round-trip, so committing it inline would generate manifest churn (and merge noise) on every poll. A gitignored `.forge/state/` sibling matches the `stateFile` cache's lifecycle exactly (rebuildable, never committed) and keeps the manifest stable. Rejected alternative: widen `Manifest.designPr` + `Piece` to carry the baseline inline — rejected for the churn reason above. No v1.3 spec edit required; v1.2 §9 is satisfied by any durable orchestrator-local store.
**In the spec today:** v1.2 §9 (poll baseline / `BaselineCursor`), §4 (manifest source-of-truth invariant). `modules/forge-core/src/main/scala/io/forge/core/paths/ForgePaths.scala` (`pollBaselineFile`), `modules/forge-git/src/main/scala/io/forge/git/watcher/PollBaseline.scala` (the persisted type).
**Implemented (slice-1.4b Task 1.4.10-d2c):** `io.forge.app.orchestrator.PollBaselineStore` (atomic temp+rename map keyed by PR number; missing/malformed → `PollBaseline.empty`) + `Orchestrator.watcherIO`, which seeds the cursor `Ref` from the store on entry and persists `decoded.nextBaseline` after each `Snapshot` poll. Pinned by `PollBaselineStoreSuite` + `OrchestratorPollBaselinePersistenceSuite`.

### S4-4. `RebuildState.run` widened to `RebuildResult(feature, inFlightSessions)`; orchestrator logs a `monitor.outcome` settle marker (slice-1.4b Task 1.4.10)
**Decision (slice-1.4b Task 1.4.10):** `RebuildState.run` now returns `IO[Either[RebuildError, RebuildResult]]` where `RebuildResult(feature: Feature, inFlightSessions: Vector[InFlightSession])`. `InFlightSession(phase: SessionPhase, sessionId: String, piece: Option[PieceId])` is a pure log-tail projection (`RebuildState.inFlightSessions(actions, feature)`): for the current state's driver phase + piece key (derived from `feature.state` — each driver-bearing state maps to exactly one `SessionPhase`), the last matching `<actor>.spawn` / `<actor>.resume` is in flight iff no `RebuildState.MonitorOutcomeKind` (`"monitor.outcome"`) action with the same piece key follows it. The orchestrator (Task 1.4.10) appends one `monitor.outcome` action per `SessionMonitor` `MonitorOutcome` (settled / settle-timeout / turn-budget / budget breach), tagged with the spawn's `piece`. On process restart the orchestrator turns each `InFlightSession` into a synthetic `FsmEvent.HarnessError` before racing any sources.
**Why this is a deviation:** Slice 1.2 (`Feature.foldEvents` / PR-E) returned only `Feature` and observed no in-flight bookkeeping; v1.2 §11.0 / §6.1 describe driver-session lifecycle but never name a log marker that distinguishes "driver still running" from "driver settled, FSM not yet advanced". The `monitor.outcome` kind is new vocabulary not enumerated in §19's action-kind list.
**Why this is OK:** The marker is needed precisely because some settle events are FSM no-ops — `Settled(Implement, Clean)` leaves the state at `PieceImplementing(p)` until `PrOpened` (Fsm.scala) — so the FSM state alone cannot tell a crashed-while-running driver (recover via NHI, worktree may carry partial changes) from a crashed-in-post-settle-window driver (replay the post-settle synthesis; `createPr` is idempotent). `monitor.outcome` is a **no-op projection** at the `Replay` layer (an unknown kind, ignored exactly like the other forensic-only kinds), so it does not perturb any §6.1 `Feature` projection or the F13 reconcile invariants — the canonical `Feature` shape is unchanged. The widened `run` ripples only to `FileStateCache.verifyAgainstLog` (`.feature`) and two test suites. v1.3 §19 should list `monitor.outcome` in the action-kind catalogue; no other spec edit required.
**In the spec today:** v1.2 §6.1 (session-id projections), §11.0 (driver lifecycle), §19 (action-kind list — `monitor.outcome` to be added in v1.3). `modules/forge-core/src/main/scala/io/forge/core/state/RebuildState.scala` (`run` / `RebuildResult` / `InFlightSession` / `inFlightSessions` / `MonitorOutcomeKind`), `modules/forge-core/src/test/scala/io/forge/core/state/RebuildStateInFlightSuite.scala` (pins the projection).

### S4-5. Reviewer process-failure retry implemented; reviewer model / cap / retry-count §18 tuning still deferred (slice-1.4b Task 1.4.10-d2c)
**Decision (slice-1.4b Task 1.4.10-d2c):** Two halves, split by where they live. (i) The **retry mechanism** (§7.6 / §11.2 step 9 "wrapped by `config.<reviewer>.reviewProcessRetries`") lands now as `io.forge.app.reviewer.RetryingReviewerCall` — a decorator over `ReviewerCall` that re-issues a one-shot on `AdapterFailure(ReviewerProcessFailure)` up to the per-method-class budget (`reviewProcessRetries` for design/PR review, `refineProcessRetries` for refine), leaving `Settled` / `Timeout` / non-process `AdapterFailure`s untouched. `OrchestratorBuilder` selects the `claude` vs `codex` §18 block by `mode` and wraps `RealReviewerCall` in it. The retry *counts* already exist on `ClaudeConfig` / `CodexConfig` (Task 1.4.9). (ii) The **reviewer model + wall-clock cap** stay hard-wired at the Task 1.4.7 / C15 v1 values (`haiku` / `gpt-5.3-codex`, 3-min cap) in `ConnectorFactory` / `Orchestrator`.
**Why this is a deviation:** v1.2 §18 has no reviewer-model / reviewer-cap / timeout-retry-policy keys, so making the model and cap configurable is a §18 schema extension, not a code-only change.
**Why this is OK:** Splitting the mechanism from the tuning lets `forge run` retry transient reviewer process failures today (the spec contract) without inventing un-spec'd config keys. Adding `claude.reviewerModel` / `reviewerCapSec` (etc.) is a `forge-design-1.3` §18 revision per the standalone-revision rule, gated on Task 1.4.16 MVP-run data showing whether a heavier reviewer at a larger cap is worth the latency (the C15 residual note + S4-3 both point here).
**Action required for v1:** Task 1.4.17 close-out decides whether the §18 reviewer-tuning extension lands in v1.3 or rolls forward; until then the v1 model/cap is the C15-validated default.
**Task 1.4.17 close-out decision (2026-05-31): rolls forward to Phase 2.** Same reasoning as [[S4-3]] — reviewer model/cap/retry tuning is an *optimisation* that wants per-run latency/cost data to tune against, and that data does not exist until Slice 2.0 instruments it. The §18 reviewer-tuning config knobs (`reviewer.model` / `reviewer.wallClockCapSec` / `reviewer.processRetries`) are therefore deferred to Phase 2 alongside the cost/latency instrumentation; the C15-validated default (claude=`haiku`, 3-min cap, one transient retry) is the v1 production reviewer.
**In the spec today:** v1.2 §7.6 (retryable process failures), §11.2 step 9 (reviewer call "wrapped by `config.<reviewer>.reviewProcessRetries`"), §18 (config — no reviewer-model/cap keys yet). `modules/forge-app/src/main/scala/io/forge/app/reviewer/RetryingReviewerCall.scala`, `modules/forge-app/src/main/scala/io/forge/app/orchestrator/{OrchestratorBuilder,ConnectorFactory}.scala`.
**Slice 2.0 close-out (2026-05-31): the measurement S4-5 was gated on now exists; tuning itself is a future Phase-2 slice, not Slice 2.0.** Slice 2.0 delivered the per-run cost/latency instrumentation (`cost.update` / `session.complete` / `forge stats`) this tuning needed to tune against — and the szork run's lived data already flagged the live target (the implement settle cap was too tight: `maxTurnCostUsd = $2.0` vs an actual $9.56 turn). But the §18 reviewer/driver model + wall-clock-cap + retry-count config knobs are still **not** wired (the C15-validated defaults remain hard-pinned); landing them is a distinct optimisation slice deferred to a later Phase-2 slice (roadmap §3.5). The same slice also carries the [[S4-3]] reviewer-cost widening and the design-2.0 §4 D3 driver-respawn-avoidance, all of which share the "tune against real attributed data" driver.

**Slice 2.2 close-out (2026-06-05): the cap re-tuning landed; the diagnosed per-turn-kill bug fixed.** The S4-5 model/cap *knobs* shipped earlier (2026-06-05, the §18 `reviewer` block, contract 1.14). Slice 2.2 (roadmap §3.5; contract [`forge-design-1.16.md`](forge-design-1.16.md) §12/§18) landed the *value* re-tuning the szork data demanded, plus the deeper finding it surfaced: **the per-turn cost cap is post-hoc, not pre-emptive** (cost is reported only in the turn-end frame, so the $2 cap could only `kill()` *after* the $9.56 turn completed — stranding an already-settled valuable turn). The fix makes the per-turn breach **non-killing** (`RealSessionMonitor.applyCaps` no longer kills; the wall-clock settle cap is the only mid-turn interrupt, the cumulative feature/piece caps the only preventive ones) and re-tunes `maxTurnCostUsd` 2.00 → 15.00 as a documented advisory threshold. The remaining S4-5 tail — reviewer process-retry *count* tuning — needs no change (the `reviewProcessRetries = 2` defaults are fine). Roadmap §3.5 `[~]` stays open pending the whole-section review.

### S4-6. The dedicated `forge new → spec → run` real-CLI `forge-it` smoke (Task 1.4.13 M12 residual) is deferred to Phase 2 (slice-1.4b Task 1.4.13 / closed-out Task 1.4.17)
**Status:** Deferred to Phase 2 test-hardening at Task 1.4.17 close-out (2026-05-31).
**Context:** Task 1.4.13 M12 shipped the per-command **unit** coverage (`CliParserSuite`, `CommandRouterSuite`, the handler suites, `MainSuite`) plus the construction-level `OrchestratorWiringSmokeSuite` (`forge-it`, `FORGE_IT_RUN_WIRING_SMOKE=1`). The one M12 piece not landed as an automated suite is a repeatable `forge new → spec → run` drive against a throwaway git repo + real `claude`/`codex`/`gh`, gated behind `FORGE_IT_RUN_*`.
**Why deferred (not a Phase-1 blocker):** the Task 1.4.16 MVP-gate run drove exactly that full CLI path — `forge new → spec → run` against a real repo (`llm4s/szork`) with real `claude`/`codex`/`gh`, reaching `FeatureDone` — and is preserved as evidence under `docs/slice-4/`. So the *behaviour* M12's smoke would assert has been exercised end-to-end once; what remains is turning that one-time manual drive into a repeatable opt-in suite. That is test-hardening, appropriate to Phase 2 (it pairs naturally with the Slice 2.0 observability work, which gives the smoke concrete cost/latency assertions to make). **Action required:** Phase 2 lands a `ForgeCliE2ESmokeSuite` in `forge-it` (`FORGE_IT_RUN_CLI_SMOKE=1`). No v1.3 spec impact.
...
### CI8. §8 CI-readiness policy is wired **orchestrator-side**; the FSM stays `CiPolicy`-agnostic (slice-1.4b Task 1.4.10b)
**Decision (slice-1.4b Task 1.4.10b, 2026-05-30):** The §8 CI-readiness policy (CI1–CI4) was specified but never wired into the orchestrator in Task 1.4.10 — `PrSnapshotDecoder` always emits `requiredChecks.required = Vector.empty`, `Fsm.ciOutcome` correctly reads an empty required-set as `Pending` (CI2), so any piece PR sat in `PieceAwaitingCi` forever with no check-discovery timeout to escape. Task 1.4.10b wires it. The split (confirmed by the pre-existing `F9CiReadinessOrderingSuite` docstring) keeps the FSM `CiPolicy`-agnostic: a new pure `io.forge.app.orchestrator.CiReadiness.evaluate` owns the policy and **populates** the snapshot's `required` set, which the orchestrator stamps onto the snapshot before forwarding `PrSnapshotUpdated`. The orchestrator runs `CiReadiness` as a self-contained sub-loop inside `pieceCiWatcherIO` with a per-visit `CiGate` ref (monotonic discovery-window anchor + consecutive-green count); the count resets by construction on every re-entry to `PieceAwaitingCi` (e.g. after a fix-up). `CiPolicy.None` stamps a degenerate all-success `required` check (and logs `ci.skipped`); `OverlaySource.Unauthorized` logs `harness.protection_unauthorized`. The discovery-failure NHI (§8 rules 1–3) flows through a new additive event `FsmEvent.CiReadinessBlocked(piece, prNumber, reason)` → `NeedsHumanIntervention(reason, ResumeAfterHumanPush(p, prNumber))` — the FSM's only new code, a dumb router that never sees policy/checks/timeouts.
**Why a new event (not `HarnessError`):** §8 mandates the `ResumeAfterHumanPush` hint, but a `PieceAwaitingCi` `HarnessError` resolves to `RunAnotherFixup` (`Fsm.nhiHintFor`). `CiReadinessBlocked` is ephemeral (only the resulting `fsm.transition` `ActionDraft` is persisted), so it needs no Replay encode/decode change. It is also the **first** producer of `NHI(ResumeAfterHumanPush)` — the resume side (`handleResume` → `PieceAwaitingCi`; CLI `--after-human-push`) already existed end-to-end.
**Rejected:** (a) **Add the policy to the pure FSM** — the FSM has no clock and no cross-poll memory; discovery-timeout / `stableGreenPolls` cannot live there, and it would couple the FSM to `CiPolicy`. (b) **Map the discovery-failure to `HarnessError`** — yields the wrong hint (`RunAnotherFixup`), a silent §8 deviation. (c) **Thread the green-count through the main `loop` ref set** — a stale count could survive a fix-up re-entry and declare premature readiness; the `watcherIO`-local ref makes the reset structural.
**Deferred to a follow-up (designed-for, not in Task 1.4.10b):** §9 base-freshness-on-readiness (spec §8 rule 4 + §9 lines 760–763: on CI readiness, `baseFreshness` → `autoUpdate ? updatePrBranch+re-poll : NHI("piece <p> PR is behind base", ResumeAfterHumanPush)`). It adds two more `SideEffects` IO seams (`baseFreshness`, `updatePrBranch`) and a second NHI reason, roughly doubling the IO/test surface for an orthogonal concern; it re-uses the `ResumeAfterHumanPush` plumbing this task establishes. Tracked as a §8/§9 carry-forward.
**In the spec today:** v1.2 §8 (CI readiness policy + discovery rules 1–4), §8.1 (epoch-scoped cache); `modules/forge-app/src/main/scala/io/forge/app/orchestrator/CiReadiness.scala` (pure policy), `Orchestrator.scala` (`pieceCiWatcherIO` / `ciPollToEvent` / `CiGate` + the `ci.skipped` / `harness.protection_unauthorized` audit drafts), `SideEffects.requiredChecksOverlay` / `RealSideEffects` (overlay fetch); `modules/forge-core/src/main/scala/io/forge/core/fsm/FsmEvent.scala` (`CiReadinessBlocked`) + `Fsm.scala` (`pieceAwaitingCiTransitions` handler). Tests: `CiReadinessSuite` (forge-app, pure policy), `F9CiReadinessOrderingSuite` + `ResumeHintCoverageSuite` (forge-core), reworked `OrchestratorE2EHappyPathSuite` (observed-only snapshots + the §8-no-checks NHI e2e), `RealSideEffectsSuite` (overlay delegation).
...
### PH4-3. Containerised worker control channel moved to TCP (Unix-socket bind mount fails on macOS) — landed in 4.3
**Decision:** the worker↔daemon control channel moves from a Unix-domain socket to **TCP** so the **containerised** topology works on the stated macOS host (contract §7). Both topologies share the TCP transport: the daemon binds an ephemeral loopback port (`0.0.0.0` in `--container` mode) and writes it to a discovery file (`instances/<name>/daemon.port`); host clients + a host-process worker read the file and connect over loopback; a containerised worker connects to `host.docker.internal:<port>` (`--add-host host.docker.internal:host-gateway` wires the name on Linux; built-in on Docker Desktop). **Originally pencilled for 4.5** but **pulled forward into Slice 4.3** so dogfood #8 runs on the macOS dev host rather than requiring a Linux box (ratified with the user 2026-06-07).
**Origin:** Task 4.3.6 dogfood prep (2026-06-07). Empirically: a container connecting to a *host-created* Unix socket over a bind mount on Docker Desktop for macOS fails with `OSError [Errno 95] Operation not supported` — the VM boundary does not pass host Unix sockets through. A socket on a *shared Docker volume* between two in-VM (Linux) containers **does** cross (verified). The original `ContainerRuntime` bind-mounted the daemon's host Unix socket into the worker container — correct on a Linux host, broken on macOS; TCP over `host.docker.internal` crosses the boundary cleanly.
**Why the suites missed it:** `ContainerRuntimeSuite` asserts the *spec* (mounts/argv, no host home), `SupervisorContainerReconcileSuite`'s opt-in real-Docker test uses only `docker run`/`wait`/`kill` (no socket crossing), and `WorkerDaemonHandshakeSuite` exercises the worker-side seams over a host-local socket (no container). Only a real container *connecting back to the daemon* — the live dogfood — surfaces the transport boundary; the host↔container crossing is not portably automatable, which is itself why this is a dogfood-class finding.
**Implemented:** `DaemonAddress` (host:port + port-file discovery) + `DaemonSocketServer`/`DaemonClient` over fs2 `Network` TCP; `Instance.socketFile` (`daemon.sock`) → `Instance.portFile` (`daemon.port`); `Daemon.serveUntilShutdown` writes/cleans the port file + a `bindHost`/`onBound` pair; `ContainerSpec.extraHosts` + the `--add-host` argv; `WorkerReporter`/`WorkerCommands` resolve the daemon by port file (host) or `--socket host:port` (container). Ratified with the user 2026-06-07 (adopt the TCP work into 4.3 + finish). Contract note in [`forge-design-2.0.md`](forge-design-2.0.md) §7 (transport) + O2/O9; plan in [`design-4.3.md`](design-4.3.md) §3 status log.
