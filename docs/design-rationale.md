...
**Watch item (still open after Slice 2.1 close):** termflow `0.4.0` (the API forge is written against) is `publishLocal`-only; Maven Central carries `0.3.0`. Before §3.4 OSS-readiness, ship `0.4.0` to Central or pin a Central-published version, else a fresh clone can't resolve the build. This is **not** closed by Task 2.1.8 — the close-out only reconciled the spec text; the resolution is a §3.4 (OSS-readiness) prerequisite. Durable home: recorded in the live spec §3.3 (the termflow dependency row carries the watch item) and roadmap §3.4.
