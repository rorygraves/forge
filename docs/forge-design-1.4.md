...
**Author:** Rory  •  **Status:** v1.4 — post-Slice-2.0 run-observability action-log additions  •  **Target:** personal tool, OSS later

**This is the implementation contract.** Earlier versions (0.1 → 0.9 → 1.0 → 1.1 → 1.2 → 1.3) and their commentaries remain in the workspace as a record of how the design evolved. Implementers read only this document.

The 0.x series captured design evolution; 1.0 was the release candidate; 1.1 folded in three Slice-0 corrections (Claude native schema, session-id preservation on resume, Codex adapter notes); 1.2 folded in three Slice-1 corrections to the §7.1 connector trait (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events); 1.3 folded in one Slice-1.4 correction to the same trait (`resumeStreamingSpec` carries the driver `systemPrompt` path — §7.1, §7.10(a) — resolving design-rationale **C14**). **1.4 — this revision — folds in the Slice-2.0 run-observability additions to the §19 action-log schema:** a new `session.complete` audit kind, an optional `wait` field on `fsm.transition`, and a new `audit.resume_from_nhi` kind (plus the operational `FORGE_DRIVER_RAW_DUMP_DIR` debug sink, §19). These were emitted by Slice 2.0 code (`cost.update` writer / `session.complete` / work-vs-wait markers / clean resume-from-NHI) ahead of the spec text; per the CLAUDE.md "spec/code deviation is a flag" rule the deviations were tracked as decisions **D1/D4/D3** in `design-2.0.md` §4 and are reconciled into the contract here. The §19 numbering and all other sections are unchanged from 1.3. Future revisions continue to be fully standalone per §23.
...
- `session.complete` — `{ phase, piece, durationMs, model, turnCostUsd, success }`. (Slice 2.0, D1 option (b).) Emitted once per settled driver session (every settle, whether the turn spent or not — contrast `cost.update`, which is emitted only when the turn actually spent). `phase` ∈ the driver phases (`spec` / `implement` / `fixup`); `durationMs` is the CLI wall-clock; `turnCostUsd` is this turn's aggregate spend. Written in the **same atomic batch** as the settle `fsm.transition` (`applyAndPersist` `extraDrafts`), so a crash after the transition still has the session record. Consumed by `forge stats` for per-phase latency / cost / turn-count rows.
...
- 1.1 was the first post-1.0 update (Slice 0 findings); 1.2 was the first post-Slice-1 update, folding in three connector-trait corrections that surfaced during the `forge-agents` implementation (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events). 1.3 — this revision — is the first post-Slice-1.4 update, folding in one further §7.1 trait correction that surfaced when the Slice-1.4 orchestrator first wired the resume path: `resumeStreamingSpec` carries the driver `systemPrompt` path so §7.10(a) prepending is honourable on Codex resumes (design-rationale C14). The interim deviation lived in `CodexConnector.resumeStreamingSpec`'s docstring and the C14 rationale entry between revisions; both are superseded by this revision and now point here. Future revisions (1.4, ...) prompted by later findings or production lessons continue the same pattern — fully standalone, sitting alongside this file as `docs/forge-design-1.x.md`.
...
This design went through nine rounds of pre-implementation review (0.1 → 0.9 → 1.0), one post-Slice-0 revision (1.0 → 1.1), one post-Slice-1 revision (1.1 → 1.2), and one post-Slice-1.4 revision (1.2 → 1.3). The reviewer was consistent across rounds. The pattern that worked:
...
The architecture stabilised at 0.5 and has remained unchanged through 1.3. Eight revisions without architectural movement is the signal that the work is done — implementation can proceed.
