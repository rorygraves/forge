...
| 2 | **No guard against a piece commit landing on a checked-out base branch.** Forge does `git checkout <piece-branch>` / `add` / `commit` / `push` on the shared worktree with no lock against concurrent external git. When the operator left `HEAD` on `main` (a `scalafmtCheckAll` verification excursion in the live worktree), Forge's piece commit landed on `main` and was pushed directly to the shared remote, bypassing the PR. | safety (operator-triggered, but a real gap) | **✅ Resolved 2026-06-04** — `RealSideEffects.assertHeadIs` now runs before *every* `git.commit` and refuses (→ `HarnessError` → NHI, no commit) when `HEAD` is not the expected design/piece/conventions branch. The operator-discipline lesson still stands (never run worktree-mutating git in a repo Forge is driving), but Forge no longer silently corrupts the base branch on the race. Plan: [`../design-3.0.md`](../design-3.0.md) T6. |
...
- **Finding #1 (§9 poll back-off) ✅ resolved 2026-06-04** (design-rationale S3-4b;
  ...
  (pre-commit `HEAD` assertion) ✅ resolved 2026-06-04** — `RealSideEffects.assertHeadIs`
  refuses to commit when `HEAD` is off the expected branch. Both dogfood-#3 maintenance
  findings are closed; the live §8.2 trigger demonstration remains the open T6 residual.
</content>
