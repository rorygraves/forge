...
**Author:** Rory  •  **Status:** v1.2 — post-Slice-1 trait-shape corrections  •  **Target:** personal tool, OSS later

**This is the implementation contract.** Earlier versions (0.1 → 0.9 → 1.0 → 1.1) and their commentaries remain in the workspace as a record of how the design evolved. Implementers read only this document.

The 0.x series captured design evolution; 1.0 was the release candidate; 1.1 folded in three Slice-0 corrections (Claude native schema, session-id preservation on resume, Codex adapter notes); **1.2 folds in three Slice-1 corrections to the §7.1 connector trait**: (a) `runStreamingSpec` / `resumeStreamingSpec` carry an initial user message — both pinned CLIs emit the init / `thread_id` event only after the first user message arrives, so the prior parameterless shape couldn't return a session with a populated `sessionId`; (b) `StreamingSession` exposes a dedicated `answerQuestion(toolUseId, answer)` for the §7.2 `tool_result` reply path — sending the answer through `send(text)` would be interpreted by Claude as a new user message rather than the awaited tool result; (c) `AgentEvent.AskUserQuestion` carries the originating `toolUseId` so the orchestrator can route the answer back. The runtime evidence and proposed delta sat in [`docs/slice-1/slice-1-findings.md`](slice-1/slice-1-findings.md) between revisions; that doc is now superseded by this revision. Future revisions continue to be fully standalone per §23.
...
3. **Session resume** via `resumeStreamingSpec(sessionId, message)` (§7.1 — the trait carries the resume `message` because both pinned CLIs require it to emit init; see design-rationale C11).
...
- Integration tests:
  ...
  - `runStreamingSpec(systemPrompt, firstUserMessage)` and `resumeStreamingSpec(sessionId, message)` exercised end-to-end against both CLIs. Assert `sessionId` is populated on return, and `oldSessionId == newSessionId` on the `<actor>.resume` event under the pinned CLIs (§6.1).
...
| Session resume contract? | `Connector.resumeStreamingSpec(sessionId, message)` returns a fresh `StreamingSession` attached to prior conversation. Both pinned CLIs need the message to elicit the init/`thread_id` event (Slice 1 F1/F2; see design-rationale C11), so the trait carries it explicitly. `runStreamingSpec` likewise takes an `initialUserMessage`. |
...
- 1.1 was the first post-1.0 update (Slice 0 findings); 1.2 — this revision — is the first post-Slice-1 update, folding in three connector-trait corrections that surfaced during the `forge-agents` implementation (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events). The interim findings doc at `docs/slice-1/slice-1-findings.md` is superseded by this revision; it remains in the tree as an evolution record alongside the slice-0 report. Future revisions (1.3, 1.4, ...) prompted by Slice 1+ findings or production lessons continue the same pattern — fully standalone, sitting alongside this file as `docs/forge-design-1.x.md`.
...
This design went through nine rounds of pre-implementation review (0.1 → 0.9 → 1.0), one post-Slice-0 revision (1.0 → 1.1), and one post-Slice-1 revision (1.1 → 1.2). The reviewer was consistent across rounds. The pattern that worked:
...
The architecture stabilised at 0.5 and has remained unchanged through 1.2. Seven revisions without architectural movement is the signal that the work is done — implementation can proceed.
