...
**Author:** Rory  •  **Status:** v1.3 — post-Slice-1.4 connector-trait correction  •  **Target:** personal tool, OSS later

**This is the implementation contract.** Earlier versions (0.1 → 0.9 → 1.0 → 1.1 → 1.2) and their commentaries remain in the workspace as a record of how the design evolved. Implementers read only this document.

The 0.x series captured design evolution; 1.0 was the release candidate; 1.1 folded in three Slice-0 corrections (Claude native schema, session-id preservation on resume, Codex adapter notes); 1.2 folded in three Slice-1 corrections to the §7.1 connector trait (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events). **1.3 — this revision — folds in one Slice-1.4 correction to the same trait:** `resumeStreamingSpec` carries the driver `systemPrompt` path (§7.1, §7.10(a)). 1.2's parameterless-of-prompt resume signature could not honour §7.10(a)'s "the same convention applies to `resumeStreamingSpec`" claim on the Codex side — a `codex exec resume` is a fresh subprocess with no path to read — so the orchestrator-side resume code surfaced the gap as design-rationale **C14**. 1.3 widens the trait (Claude ignores the path; Codex re-prepends the block) rather than dropping the §7.10(a) claim, keeping connector-specific framing inside the connector. The interim deviation lived in `CodexConnector.resumeStreamingSpec`'s docstring and design-rationale C14 between revisions; both now point here. Future revisions continue to be fully standalone per §23.
