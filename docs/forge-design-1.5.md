...
**Author:** Rory  •  **Status:** v1.5 — post-Slice-2.0 + driver-respawn-avoidance (§3.5 D3) additions  •  **Target:** personal tool, OSS later

**This is the implementation contract.** Earlier versions (0.1 → 0.9 → 1.0 → 1.1 → 1.2 → 1.3 → 1.4) and their commentaries remain in the workspace as a record of how the design evolved. Implementers read only this document.

The 0.x series captured design evolution; 1.0 was the release candidate; 1.1 folded in three Slice-0 corrections (Claude native schema, session-id preservation on resume, Codex adapter notes); 1.2 folded in three Slice-1 corrections to the §7.1 connector trait (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events); 1.3 folded in one Slice-1.4 correction to the same trait (`resumeStreamingSpec` carries the driver `systemPrompt` path — §7.1, §7.10(a) — resolving design-rationale **C14**); 1.4 folded in the Slice-2.0 run-observability additions to the §19 action-log schema (a new `session.complete` audit kind, an optional `wait` field on `fsm.transition`, a new `audit.resume_from_nhi` kind, plus the `FORGE_DRIVER_RAW_DUMP_DIR` debug sink). **1.5 — this revision — folds in the roadmap §3.5 driver-respawn-avoidance (the "D3" chunks, design-3.5.md):** a new §7.1 `resumeHeadlessDriver` connector method (the headless driver-side analogue of `resumeStreamingSpec`, extended to §7.10(a)), the §11.4 restart-recovery **resume-instead-of-respawn** behaviour gated by a worktree-safety classifier (default-on once safe), the §19 `<actor>.resume` kind now emitted for piece-driver restart resumes too, and a `resumed` field on `session.complete` so `forge stats` measures the avoided re-exploration (gap #10). These were built by the D3-1…D3-4 chunks ahead of the spec text and tracked as design-rationale **C19** + the `design-3.5.md` status log per the CLAUDE.md "spec/code deviation is a flag" rule; they are reconciled into the contract here. The §18 reviewer/driver tuning track (design-rationale S4-3 / S4-5) is still pending and folds into a later revision. All section numbering and all other sections are unchanged from 1.4. Future revisions continue to be fully standalone per §23.
...
- 1.1 was the first post-1.0 update (Slice 0 findings); 1.2 was the first post-Slice-1 update, folding in three connector-trait corrections that surfaced during the `forge-agents` implementation (initial user message on streaming spawn/resume; `answerQuestion` for the `tool_result` reply path; `toolUseId` on `AskUserQuestion` events). 1.3 was the first post-Slice-1.4 update, folding in one further §7.1 trait correction that surfaced when the Slice-1.4 orchestrator first wired the resume path: `resumeStreamingSpec` carries the driver `systemPrompt` path so §7.10(a) prepending is honourable on Codex resumes (design-rationale C14). 1.4 was the post-Slice-2.0 update, folding the run-observability §19 action-log additions (`session.complete`, `fsm.transition` `wait`, `audit.resume_from_nhi`). 1.5 — this revision — folds in the Phase-2 roadmap §3.5 driver-respawn-avoidance work (design-3.5.md, chunks D3-1…D3-4): the §7.1 `resumeHeadlessDriver` method, the §11.4 restart-recovery resume-instead-of-respawn behaviour, the §19 `<actor>.resume` piece-driver emission, and the `session.complete` `resumed` field. The interim deviations lived in design-rationale **C19** and the `design-3.5.md` status log between revisions; both are superseded by this revision and now point here. Future revisions prompted by later findings or production lessons continue the same pattern — fully standalone, sitting alongside this file as `docs/forge-design-1.x.md`.
...
The architecture stabilised at 0.5 and has remained unchanged through 1.5. Ten revisions without architectural movement is the signal that the work is done — implementation can proceed.
