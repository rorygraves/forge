...
- **O9 — Worker↔daemon transport:** a **dedicated worker control channel**, kept separate from the client JSON-RPC, carrying worker/daemon control, budget auth (B2), event export (B3), and credential brokering (O6).
...
- **O11 — Reservation estimate granularity.** Per-session-cap as the reservation amount (simple, coarse) vs. a learned/profiled estimate per phase (tighter aggregate packing, more machinery). Finalization corrects either from the real `cost.update`.
