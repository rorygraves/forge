...
│  │  • status + control API over TCP loopback (§6); TUI & CLI are clients ││
│  │                                                                       ││
...
- **6.1 Supervisor.** Long-running process; TUI and CLI are clients over a **loopback TCP socket** (an ephemeral port the daemon writes to a discovery file beside the instance lock — moved off a Unix-domain socket in the Slice-4.3 TCP migration so a containerised worker can reach the daemon over `host.docker.internal`). CLI stays the primary scripting interface. One daemon per instance; it owns the **instance lock** (generalising v1 `ProcessLock`); per-worker locks live below it.
...
- **Implemented (4.3).** The supervisor runs `forge worker --instance <name> --worker-id <id> --repo <repo> --feature <id> --worker-root <path> --socket <host:port> --container` **inside** the container. Exactly **one** bind mount — the isolated clone *worker root* (RW; the checkout is `<root>/checkout`) — and **no host home mount, no daemon-socket mount, no secret env in the spec** (`docker inspect`-safe, O6); the worker reaches the daemon over **TCP** at `host.docker.internal:<port>` (passed as `--socket`, with `--add-host host.docker.internal:host-gateway` so the name resolves) and brokers its credentials over that connection (`broker-credentials`: a per-repo scoped `gh` token + best-effort agent keys, sourced via a `SecretSource` seam, **refusing** rather than falling back to a host login). The image resolves from the clone's committed `Forgefile.image` (O1), else the fallback **`forge-worker:latest`** — a forge-capable base bundling `forge` + the pinned `gh`/`claude`/`codex` tooling (the default image + build wiring live in `docker/forge-worker/`). Containers are spawned `--rm`-free so a restarted daemon can still inspect/await an exited one. The abstract seam is `OciRuntime` (`run`/`running`/`attach`); the Docker-first impl shells `docker run -d`/`wait`/`kill`/`rm -f`. Defence-in-depth non-root + host-UID mapping is a 4.5 carry-forward.
  - **Transport (TCP control channel, landed in 4.3).** The worker reaches the daemon over **TCP**, not a bind-mounted Unix socket. Origin: the 4.3.6 dogfood prep found a container connecting to a *host-created* Unix socket over a bind mount fails on Docker Desktop for **macOS** (the stated host, §7) with `Errno 95 Operation not supported` (the VM boundary does not pass host Unix sockets through; only a socket on a *shared Docker volume* between two in-VM containers crosses). Rather than restrict the container topology to Linux, the control channel was **moved to TCP** in Slice 4.3 (originally pencilled for 4.5): the daemon binds an ephemeral loopback port (`0.0.0.0` in `--container` mode so the container can reach it) and writes it to a discovery file; host clients read the file and connect over loopback; the containerised worker connects to `host.docker.internal:<port>`. This crosses the macOS VM boundary cleanly, so **dogfood #8 runs on macOS**. The host-process (4.2) topology uses the same TCP transport over loopback. See PH4-3.
...
- **O2 — Wire protocol:** JSON-RPC 2.0 over TCP *(loopback; an ephemeral port the daemon writes to a discovery file — moved off a Unix socket in the Slice-4.3 TCP migration, see O9 / PH4-3)*.
...
- **O9 — Worker↔daemon transport:** a **dedicated worker control channel**, kept separate from the client JSON-RPC, carrying worker/daemon control, budget auth (B2), event export (B3), and credential brokering (O6). *Implemented over **TCP** (Slice 4.3): a host-process worker connects over loopback; a containerised worker connects to `host.docker.internal:<port>`, which crosses the Docker Desktop macOS VM boundary cleanly (a bind-mounted host Unix socket does not — see §7 transport note / PH4-3). The TCP migration was pulled forward from 4.5 so the container topology — and dogfood #8 — works on the stated macOS host.*
