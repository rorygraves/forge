...
> **🟡 open 2026-06-01; implementation exit met pending close-out.** Per-Task breakdown + audit trail:
...
> Landed so far: termflow `0.4.0` wiring + repo-wide Scala 3.7.1 bump, the
> pure `TuiSnapshot` builder, read-only `forge tui <feature>` command, polling
> refresh, scrollable log tail, Q&A display, resize-aware themed panes, and `?`
> key-help overlay. Open items: Task 2.1.8 section close-out / spec revision; Task
> 2.1.5 live `AgentEvent` tap is carried forward unless real use shows per-settle
> log refresh is too coarse. Open watch item: termflow `0.4.0` is
> `publishLocal`-only (Central has `0.3.0`) — resolve before §3.4 OSS-readiness.
