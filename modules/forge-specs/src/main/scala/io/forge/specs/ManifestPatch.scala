...
enum ManifestPatchOp derives ReadWriter:
  /** Insert a new piece. `after = None` means "at the head of the list"; with
    * any merged pieces present the resulting manifest fails `Manifest.validate`
    * (§5.5), and `applyTo` surfaces that as an op-local error.
    *
    * `after = Some(p)` inserts immediately after `p`. The op rejects `p`
    * pointing to a merged piece that isn't the last merged piece, because the
    * insertion would split the merged prefix. */
  ...
  /** Replace the piece ordering. Must be a duplicate-free permutation of the
    * current piece ids and must preserve the merged prefix (§5.5). */
    ...
    def applyTo(m: Manifest): Either[String, Manifest] = op match
      case ManifestPatchOp.AddPiece(after, piece) =>
        if m.pieces.exists(_.id == piece.id) then
          Left(s"piece '${piece.id.value}' already exists")
      ...
      case ManifestPatchOp.ReorderPieces(newOrder) =>
        ...
        if newOrder.distinct.size != newOrder.size then
          Left("contains duplicates")
        else if newOrder.size != currentIds.size then
          Left(s"size ${newOrder.size} does not match current ${currentIds.size}")
        else if newOrder.toSet != currentIds.toSet then
          Left("not a permutation of current piece ids")
        else
...
/** §6 — a named bundle of ops applied as a unit. `validate` and `applyTo`
  ...
  * ops produced, and the post-op manifest is re-checked against
  * `Manifest.validate` before the next op runs.
  *
  * This is what catches internally contradictory patches — two AddPiece for
  * the same id, RemovePiece(p3) followed by EditPiece(p3), etc. — that a
  * one-shot validator against the original manifest would let through. */
final case class ManifestPatch(reason: String, ops: Vector[ManifestPatchOp]) derives ReadWriter:

  /** Apply each op in order. On any failure (op-local error or post-op
    * `Manifest.validate` rejection) stop, prefix the error(s) with the failing
    * op's index/kind, and return them; remaining ops are not attempted. */
  ...
  /** Run `applyTo` without committing the result — the gate used by
    * `forge reconcile` and the §14.3 refinery flow. */
