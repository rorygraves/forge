...
class RealGitClientCommitSuite extends munit.FunSuite:
  ...
  fixture.test("commit bypasses a failing pre-commit hook (--no-verify) — local dev hooks don't gate Forge"): work =>
    // Target repos may install a pre-commit hook that runs the full formatter/build (szork runs sbt scalafmtCheck);
    // Forge's commit must not be blocked by it (the remote CI gate is the verification).
    val hook = work / ".git" / "hooks" / "pre-commit"
    os.write.over(hook, "#!/bin/sh\necho 'pre-commit hook intentionally failing' >&2\nexit 1\n", createFolders = true)
    os.proc("chmod", "+x", hook.toString).call(cwd = work)
    os.write(work / "h.txt", "hooked\n")
    val client = RealGitClient(work)
