...
/** §5.2 slug algorithm and collision suffix logic.
  *
  * The slugger is pure; the existence check is injected so the same code powers both `forge new` (against the
  * filesystem) and tests (against an in-memory set).
  */
object FeatureIdSlugger:
  ...
  /** §5.2 collision rule. The first slug that `exists` returns `false` for is the chosen one. Suffix `-2, -3, ...` is
    * appended to the base slug only.
    */
