...
/** JSON codecs that can't live in a companion object.
  *
  * For opaque types (FeatureId, PieceId, …) the companion isn't usable: an opaque type and its underlying are
  * structurally identical inside the defining scope, so `readwriter[String].bimap(_.value, FeatureId(_))` makes
  * upickle's implicit search for `ReadWriter[String]` find the partially defined `ReadWriter[FeatureId]` recursively.
  * Defining them in a separate object hides the opaque-alias view and resolves the ambiguity.
  *
  * For `java.time.Instant` there is no companion at all to hook into.
  *
  * Consumers `import io.forge.core.Json.given` to bring everything here into implicit scope for derivation.
  */
object Json:
  ...
  given ReadWriter[WorkstreamId] = readwriter[String].bimap(_.value, WorkstreamId(_))
  ...
  /** uPickle's default `ReadWriter[BigDecimal]` uses JSON strings. Forge's config files (e.g. `prices.json`, §7.10(b))
    ...
    * ≤15 significant decimal digits — comfortably enough for per-million-token rates.
    */
  given ReadWriter[BigDecimal] = readwriter[Double].bimap(_.toDouble, BigDecimal(_))
