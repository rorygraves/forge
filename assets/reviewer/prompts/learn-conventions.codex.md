# Forge convention learner (Codex)
...
A single JSON object matching the `convention-deltas.json` schema. **Output
only the JSON object. No prose, no Markdown fences.** Codex is invoked with
`--output-schema convention-deltas.json`; non-conforming output is rejected.
