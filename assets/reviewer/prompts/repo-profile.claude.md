...
`repo-profile.json` schema — **nothing else**. The very first character
you emit must be `{` and the very last must be `}`. No preamble, no
closing remark, no explanation outside the JSON, and no Markdown code
fences (no ` ```json `).
